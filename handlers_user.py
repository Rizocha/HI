...
# ── obuna tekshiruv ───────────────────────────────────────────────────────────
async def sub_gate(bot: Bot, uid: int, msg: Message = None, cb: CallbackQuery = None) -> bool:
    ok, missing = await check_sub(bot, uid)
    if ok:
        return True
    ext  = db.get_ext_channels()
    text = db.get_text("sub_req")
    markup = kb.sub_kb(missing, ext)
    if cb:
        try:
...
async def send_video(target: Message, bot: Bot, code: str, uid: int):
    ...
    try:
        ...
        cap = (
            ...
            f"📝 <b>Tavsif:</b> {m.get('description') or 'Yoq'}\n\n"
            f"🤖 @{me.username}"
        )
        ...
        await target.answer(f"Yuborishda xatolik. Keyinroq urinib ko'ring.")
...
async def cmd_start(msg: Message, bot: Bot):
    ...
    if len(args) > 1:
        ...
        if not await sub_gate(bot, msg.from_user.id, msg=msg):
            return
...
async def cb_checksub(call: CallbackQuery, bot: Bot):
    ...
    if ok:
        await call.message.edit_text(
            db.get_text("sub_ok"),
            reply_markup=kb.back_kb("main")
        )
    else:
        await call.answer("Hali barcha kanallarga obuna bolmagansiz!", show_alert=True)
...
async def cb_main(call: CallbackQuery):
    text = db.get_text("start", name=call.from_user.first_name)
    try:
        await call.message.edit_text(text, reply_markup=kb.user_menu(), parse_mode="HTML")
    except:
        await call.answer()
...
async def cb_list(call: CallbackQuery, bot: Bot):
    if not await sub_gate(bot, call.from_user.id, cb=call):
        return
    movies = db.all_movies()
    if not movies:
        await call.message.edit_text(db.get_text("no_movies"), reply_markup=kb.back_kb())
        return
    await call.message.edit_text(
...
async def cb_fav(call: CallbackQuery):
    ...
    await call.answer("Sevimlilarga qoshildi!" if added else "Sevimlidan olindi!")
...
async def cb_cats(call: CallbackQuery, bot: Bot):
    ...
    cats = db.categories()
    if not cats:
        await call.message.edit_text("Janrlar yoq.", reply_markup=kb.back_kb())
        return
...
async def cb_cat(call: CallbackQuery, bot: Bot):
    ...
    if not movies:
        await call.message.edit_text(
            f"<b>{cat}</b> janrida kinolar yoq.",
            reply_markup=kb.back_kb("u:cats"), parse_mode="HTML"
        )
        return
    await call.message.edit_text(
...
async def cb_favs(call: CallbackQuery, bot: Bot):
    ...
    movies = db.get_favs(call.from_user.id)
    if not movies:
        await call.message.edit_text("Sevimlilar bosh.", reply_markup=kb.back_kb())
        return
    await call.message.edit_text(
        f"❤️ <b>Sevimlilar</b> — {len(movies)} ta:",
        reply_markup=kb.movie_list_kb(movies, back="main", cb_prefix="play"),
        parse_mode="HTML"
    )
...
async def cb_hist(call: CallbackQuery, bot: Bot):
    ...
    movies = db.get_history(call.from_user.id)
    if not movies:
        await call.message.edit_text("Korish tarixi bosh.", reply_markup=kb.back_kb())
        return
    await call.message.edit_text(
...
async def cb_search(call: CallbackQuery):
    ...
    await call.message.edit_text(
        ...
        reply_markup=kb.cancel_kb("cancel")
    )
...
async def handle_text(msg: Message, bot: Bot):
    ...
    if text in btn_map:
        ...
        if action == "list":
            movies = db.all_movies()
            if not movies:
                await msg.answer(db.get_text("no_movies"))
                return
            await msg.answer(
                f"🎬 <b>Kinolar</b> — {len(movies)} ta:",
                reply_markup=kb.movie_list_kb(movies), parse_mode="HTML"
            ...
            if not movies:
                await msg.answer("Sevimlilar bosh.")
                return
            await msg.answer(
                f"❤️ <b>Sevimlilar</b> — {len(movies)} ta:",
            ...
            if not top:
                ...
                return
            lines = "\n".join(
                f"{i+1}. <b>{m['title']}</b> — {m.get('views',0):,} 👁"
                for i, m in enumerate(top)
            )
    ...
    if st["s"] == "searching":
        ...
        if not results:
            await msg.answer(db.get_text("not_found"))
            return
        if len(results) == 1:
            await send_video(msg, bot, results[0]["code"], msg.from_user.id)
            return
        await msg.answer(
